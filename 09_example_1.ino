...
#define _MEDIAN_NUM 30   // 중위수 필터 크기 (N값)
...
public:
    Queue() : front(0), rear(-1), count(0) {
        for (int i = 0; i < _MEDIAN_NUM; i++) {
            arr[i] = (_DIST_MIN+_DIST_MAX)/2;  // 모든 요소를 -1로 초기화
        }
    }

    bool isFull() {
        return count == _MEDIAN_NUM;
    }
    ...
    void pop() {
        int value = arr[front];
        front = (front + 1) % _MEDIAN_NUM;
        count--;
    }

    float getMedian() {
        int sortedArr[_MEDIAN_NUM];
        int size = count;

        for (int i = 0; i < size; i++) {
            sortedArr[i] = arr[(front + i) % _MEDIAN_NUM];
        }
...
void loop() {
  ...
  dist_median = dist_prevs.getMedian();
  ...
  if(dist_prevs.isFull()) dist_prevs.pop();
  dist_prevs.push(dist_raw);
